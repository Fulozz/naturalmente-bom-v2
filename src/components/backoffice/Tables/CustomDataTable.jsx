...
const CustomDataTable = () => {
  ...
  // Show the select data, and handle the page Index
    ...
    <div className='bg-slate-50 dark:bg-slate-700 text-emerald-700 dark:text-slate-50 p-8 rounded-lg shadow-xl'>
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between p-4" aria-label="Table navigation">
            <span className="text-base font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">Showing <span className="font-semibold text-gray-900 dark:text-white">{itemStartIndex}-{itemEndIndex}</span> of <span className="font-semibold text-gray-900 dark:text-white">{itemTotalIndex}</span></span>
            <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                ...
                </li>
                {
                  Array.from({length:numberOfPages}, (_, i)=> {
                    return(
                      <li key={i}>
                          <button onClick={()=> setCurrentPage(i + 1)} disabled={currentPage == i + 1} className={currentPage == i + 1 ? "flex items-center justify-center px-3 h-10 leading-tight text-emerald-700 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-50 dark:hover:bg-gray-700 dark:hover:text-white" : "flex items-center justify-center px-3 h-10  leading-tight text-gray-100 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"} >{i + 1}</button>
                      </li>
                    )
                  })
                }
                <li>
                    <button onClick={()=> setCurrentPage(currentPage + 1)} disabled={currentPage == numberOfPages} className="flex items-center justify-center px-3 h-10  leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</button>
                </li>
            </ul>
        </nav>
    </div>
